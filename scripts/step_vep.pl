...
#$cmd = "--force_overwrite --cache --offline --species homo_sapiens --merged --fork $ncpu --domains --af --pubmed --check_existing --biotype --regulatory --hgvs --numbers --symbol --canonical --sift b --polyphen b --flag_pick --pick_order biotype,rank,canonical,appris,tsl,ccds,length,mane --af_gnomad --vcf --use_transcript_ref --plugin SpliceRegion --plugin CADD,$cad --custom $clinvar_vcf,ClinVar,vcf,exact,0,CLNSIG,CLNREVSTAT,CLNDN --custom $gnomad_vcf,gnomAD3,vcf,exact,0,AF,AF_female,AF_male,AF_afr,AF_afr_female,AF_afr_male,AF_ami,AF_ami_female,AF_ami_male,AF_amr,AF_amr_female,AF_amr_male,AF_asj,AF_asj_female,AF_asj_male,AF_eas,AF_eas_female,AF_eas_male,AF_fin,AF_fin_female,AF_fin_male,AF_nfe,AF_nfe_female,AF_nfe_male,AF_sas,AF_sas_female,AF_sas_male,AF_oth,AF_oth_female,AF_oth_male --dir $index_dir --fasta $fasta --I $dir_run/$cohort.$split.vep_in.vcf --stats_text --stats_file $dir_run/$cohort.$split.vep_stats.txt -o stdout | $bgzip_bin -c >$dir_run/$cohort.$split.vep.vcf.gz";
$cmd = "--force_overwrite --cache --offline --species homo_sapiens --merged --fork $ncpu --domains --af --pubmed --check_existing --biotype --regulatory --hgvs --numbers --symbol --canonical --sift b --polyphen b --flag_pick --pick_order rank,biotype,canonical,appris,tsl,ccds,length,mane --af_gnomad --vcf --use_transcript_ref --plugin SpliceRegion --plugin CADD,$cad --custom $clinvar_vcf,ClinVar,vcf,exact,0,CLNSIG,CLNREVSTAT,CLNDN --custom $gnomad_vcf,gnomAD3,vcf,exact,0,AF,AF_female,AF_male,AF_afr,AF_afr_female,AF_afr_male,AF_ami,AF_ami_female,AF_ami_male,AF_amr,AF_amr_female,AF_amr_male,AF_asj,AF_asj_female,AF_asj_male,AF_eas,AF_eas_female,AF_eas_male,AF_fin,AF_fin_female,AF_fin_male,AF_nfe,AF_nfe_female,AF_nfe_male,AF_sas,AF_sas_female,AF_sas_male,AF_oth,AF_oth_female,AF_oth_male --dir $index_dir --fasta $fasta --I $dir_run/$cohort.$split.vep_in.vcf --stats_text --stats_file $dir_run/$cohort.$split.vep_stats.txt -o stdout | $bgzip_bin -c >$dir_run/$cohort.$split.vep.vcf.gz";
$cmd =~ s/\s+-/ \\\n  -/g;
$cmd = "$vep_bin $cmd";
#warn "$cmd\n"; exit(PIPE_NO_PROGRESS);
